...
def boundingbox_refinement(V, ngpu=None, batchsize=None, strict=True):
    ...
    B = vipy.batch.Batch(V, ngpu=ngpu, strict=False)
    ...
    V = [v for v in V if (v is not None) and ((strict == False) or (not v.hasattribute('unrefined')))]  # remove videos that failed refinement
