...
class ActivityRecognition(object):
    ...
    def int2cls(self, index=None):
        d = {v:k for (k,v) in self.cls2int().items()}
    ...
    def classlist(self):
        return [k for (k,v) in sorted(list(self.cls2int().items()), lambda k,v: v)]

    def fromindex(self, k):
        int2cls = {v:k for (k,v) in self.cls2int().items()}
        assert k in int2cls, "Invalid class index '%s'" % (str(k))
        return int2cls[k]

    def label_confidence(self, video=None, tensor=None, threshold=None):
        logits = self.__call__(video, tensor)
        conf = [[(self.int2cls(j), s[j]) for j in i[::-1] if threshold is None or s[j]>threshold] for (s,i) in zip(logits, np.argsort(logits, axis=1))]
    ...
    def topk(self, k, video=None, tensor=None, threshold=None):
        logits = self.__call__(video, tensor)
        topk = [[self.int2cls(j) for j in i[-k:][::-1] if threshold is None or s[j] >= threshold] for (s,i) in zip(logits, np.argsort(logits, axis=1))]
