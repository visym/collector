...
class PIP_370k(PIP_250k):
    ...
    def __init__(self, pretrained=True, deterministic=False, modelfile=None, mlbl=False, mlfl=False):
        ...
        self._class_to_weight = {'car_drops_off_person': 0.6577909684528728, 'car_moves': 0.12317994996386322, 'car_picks_up_person': 0.6579534060299695, 'car_reverses': 0.6668457809331401, 'car_starts': 0.5356619054520227, 'car_stops': 0.5271828883494977, 'car_turns_left': 1.048556598865943, 'car_turns_right': 0.633471782624432, 'hand_interacts_with_person': 0.3327358498555556, 'person_abandons_package': 0.9503771420432894, 'person_carries_heavy_object': 0.49533041786918786, 'person_closes_car_door': 0.4368197914121422, 'person_closes_car_trunk': 0.6568989935203061, 'person_closes_facility_door': 0.4352499089632217, 'person_embraces_person': 0.6038260210126598, 'person_enters_car': 0.6253684585655115, 'person_enters_scene_through_structure': 0.32613774621682623, 'person_exits_car': 0.6040998339685664, 'person_exits_scene_through_structure': 0.39885962840095235, 'person_interacts_with_laptop': 0.568583507835758, 'person_loads_car': 0.7364240789713548, 'person_opens_car_door': 0.4453254751326027, 'person_opens_car_trunk': 0.6557672453158654, 'person_opens_facility_door': 0.37165327349956223, 'person_picks_up_object': 0.41559543249389513, 'person_purchases_from_cashier': 5.318128378679364, 'person_purchases_from_machine': 5.296982739002706, 'person_puts_down_object': 0.2916674677305267, 'person_reads_document': 0.5018141666293175, 'person_rides_bicycle': 1.4655568304281417, 'person_sits_down': 0.4878480852729766, 'person_stands_up': 0.49852789179873913, 'person_steals_object': 0.9047138600062349, 'person_talks_on_phone': 0.24551993344253237, 'person_talks_to_person': 0.24717123407564, 'person_texts_on_phone': 0.4873572924306496, 'person_transfers_object_to_car': 2.955498965855088, 'person_transfers_object_to_person': 0.5934697221780514, 'person_unloads_car': 0.5489610214728261, 'person_walks': 6.247086325248209}
...
class ActivityTracker(PIP_250k):
    ...
    def __call__(self, vi, topk=1, activityiou=0, mirror=False, minprob=0, trackconf=0.1, maxdets=None, lr_threshold=-2.0, lr_merge_threshold=-2.0, avgdets=None):
        ...
        try:
            ...
            dstbox = {k:v.track(a.actorid()).boundingbox(a.startframe(), a.endframe()) for (k,a) in v.activities().items() if a.category() in categories}  # precompute
            srcbox = {k:bb.clone().maxsquare().dilate(1.2) for (k,bb) in dstbox.items()}                            
            v.activitymap(lambda a: a.confidence(0.1*a.confidence()) if (a._label in categories and
            ...
            dstbox = {k:v.track(a.actorid()).boundingbox(a.startframe(), a.endframe()) for (k,a) in v.activities().items() if (a.category().startswith('person') and ('vehicle' in a.category() or 'trunk' in a.category()))}  # precompute            
            ...
            v.activitymap(lambda a: a.confidence(0.1*a.confidence()) if ((a.category().startswith('vehicle') and ('person' in a.category())) and not any([t.category() == 'person' and t.segment_maxiou(v.track(a._actorid), a._startframe, a._endframe) > 0 for t in v.tracks().values()])) else a)

            # Vehicle/Person track: vehicle dropoff/pickup must be accompanied by an associated person track start/end
            v.activitymap(lambda a: a.confidence(0.1*a.confidence()) if (a.category() == 'vehicle_drops_off_person' and not any([t.category() == 'person' and t.segment_maxiou(v.track(a._actorid), t.startframe(), t.startframe()+1) > 0 for t in v.tracks().values()])) else a)
            v.activitymap(lambda a: a.confidence(0.1*a.confidence()) if (a.category() == 'vehicle_picks_up_person' and not any([t.category() == 'person' and t.segment_maxiou(v.track(a._actorid), t.endframe()-1, t.endframe()) > 0 for t in v.tracks().values()])) else a)
            ...
            for a in mergeable_dets:
                for o in mergeable_dets:
                    if ((o._startframe >= a._startframe) and (a._id != o._id) and (o._actorid == a._actorid) and (o._label == a._label) and (o._id not in merged) and (a._id not in merged) and
                        ((a.temporal_distance(o) <= self.temporal_support() or (a.category() in briefmerge and a.temporal_distance(o) < 10*v.framerate())))):
            ...
            suppressed = set([])
            activitybox = {a.id():v.track(a.actorid()).boundingbox(a.startframe(), a.endframe()) for a in v.activities().values() if a.category() in tosuppress}
