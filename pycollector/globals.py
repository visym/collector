...
def backend(version=None, flush=False):
    if version is not None:
        assert version in ['v1', 'v2', 'dev', 'test']
        GLOBALS['BACKEND_VERSION'] = version
        
    if GLOBALS['BACKEND_VERSION'] == 'v1' and (version not in GLOBALS['BACKEND'] or flush):
        GLOBALS['BACKEND'][version] = pycollector.backend.Prod_v1()
    elif GLOBALS['BACKEND_VERSION'] == 'v2' and (version not in GLOBALS['BACKEND'] or flush):
